...
const toggleThemeBtn = document.getElementById("toggleTheme"); toggleThemeBtn?.addEventListener("click", () => { const isLight = document.body.classList.toggle("light-theme"); localStorage.setItem(THEME_KEY, isLight ? "light" : "dark"); dropdown.classList.add("hidden"); });
...
function makeSessionTitle(htmlContent) { const div = document.createElement("div"); div.innerHTML = htmlContent || ""; const userEl = div.querySelector(".user-msg"); if (userEl && userEl.textContent.trim()) { let txt = userEl.textContent.trim(); if (txt.length > 40) txt = txt.slice(0, 40) + "..."; return txt; } return "Chat - " + new Date().toLocaleString(); }

function newChatAction() { const currentHTML = chatMessages.innerHTML.trim(); if (!currentHTML) { chatMessages.innerHTML = ""; localStorage.removeItem(HISTORY_KEY); dropdown.classList.add("hidden"); return alert("Mulai chat baru."); } const sessionsRaw = localStorage.getItem(SESSIONS_KEY); const sessions = sessionsRaw ? JSON.parse(sessionsRaw) : []; const session = { id: Date.now(), title: makeSessionTitle(currentHTML), html: currentHTML, createdAt: new Date().toISOString(), }; sessions.unshift(session); localStorage.setItem(SESSIONS_KEY, JSON.stringify(sessions)); chatMessages.innerHTML = ""; localStorage.removeItem(HISTORY_KEY); dropdown.classList.add("hidden"); alert("Chat diarsipkan sebagai: " + session.title); } newChatBtn?.addEventListener("click", () => { if (confirm("Mulai chat baru? Riwayat akan dihapus.")) newChatAction(); });

clearChatBtn?.addEventListener("click", () => { if (!confirm("Yakin hapus semua pesan?")) return; chatMessages.innerHTML = ""; localStorage.removeItem(HISTORY_KEY); dropdown.classList.add("hidden"); });
...
// ✅ Efek mengetik seperti ChatGPT function appendBotMessage(text) { const botMsg = document.createElement("div"); botMsg.className = "bot-msg"; chatMessages.appendChild(botMsg);
...
async function sendMessageToBot(message) { if (!message || !message.trim()) return; appendUserMessage(message); saveHistory(); chatMessages.scrollTop = chatMessages.scrollHeight;
...
function startChatFromWelcome() { const msg = firstMsg?.value?.trim(); if (!msg) return; showChatUI(); setTimeout(() => sendMessageToBot(msg), 120); }
...
sendBtn?.addEventListener("click", () => { const txt = userInput?.value || ""; if (!txt.trim()) return; sendMessageToBot(txt); userInput.value = ""; });

userInput?.addEventListener("keydown", (e) => { if (e.key === "Enter") { const txt = userInput?.value || ""; if (!txt.trim()) return; sendMessageToBot(txt); userInput.value = ""; } });

[newChatBtn, changeTokenBtn, clearChatBtn].forEach((btn) => { btn?.addEventListener("click", () => dropdown.classList.add("hidden")); }); });